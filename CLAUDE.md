...
7. **Development applications: link + factual metadata only, never full reproduction.** Surface, per application, *only* Council's own basic facts from the Development.i register — application number, property locality (suburb), lodged/received date, current status, Council's one-line proposal description, and a deep link to Council's own detail page for that application. Do NOT store or show the assessing officer's name (`project_officer`), the officer report or recommendation, Council's decision narrative (`decision_desc`), appeal results (`appeal_result`), or public submissions / submitter details (`submissionindicator`, `publicnotification`). Status ("In Progress" / "Decided" / "Past") is fine; the *outcome narrative* is not. **When in doubt, exclude** — the safe line is "a DA exists, here are its basic facts, go to Council for the detail." `scrape/development_applications.py` **whitelists** its output so an excluded field can't leak (same discipline as Shape Your Ipswich). Council's Development.i is now the authoritative factual source we surface; **PlanningAlerts stays as the complement** — the "email me when someone applies to build near my street" alert — and `_planningalerts_html()` keeps that pointer on every street and suburb page. The geo/map endpoint returns only Council's *mapped* subset, so pages link to the full register and never assert a total DA count. Both halves still matter — the About page states this boundary.
...
| Shape Your Ipswich consultations | `www.shapeyouripswich.com.au/ccm/the_hive_projects/tools/the_hive_projects_list/load_more/{blockID}` | **Working** — `scrape/shape_your_ipswich.py`. Granicus EngagementHQ ("the Hive"). No public REST API, but the `/projects` "Show more" button calls a JSON load-more endpoint per list block (Open/Active/Closed) that returns full project metadata — enumerate via those, not the HTML cards or sitemap. Listing JSON only, never the project pages: invariant 8 (project pages are surveys/comments/forums) and the page's only clean text is its `og:description` = the feed summary anyway. See docs/notes.md. |
...
| Development applications (Development.i) | `developmenti.ipswich.qld.gov.au` — `POST /Geo/GetApplicationFilterResults` (GeoJSON) | **Working** — `scrape/development_applications.py`. Link + factual-metadata layer only (see Invariant 7). Anti-forgery protected: GET `/` first for the `.AspNetCore.Antiforgery` cookie + hidden `__RequestVerificationToken`, then send the token as a header on the POST (else 500). Body uses `SortAscending` (bool), not `SortDirection`. Enumerate localities via `GET /Geo/GetLocality` and vary `LocalityId` (the geo feature carries no suburb; LocalityId gives the bucket for free). **The geo endpoint is the map layer and returns only ~4% of the full register** (Swanbank: ~14 mapped vs 345 in Council's list endpoint) — surfaced as Council's mapped set with a link to the full register, never a claimed total. Output whitelisted; excluded fields can't leak. See docs/notes.md. |
...
- Don't rename URL slugs once published — they get indexed by Google. **This is enforced, not aspirational**: `data/url_registry.json` pins each entity's slug to a stable Council id (projects have a per-feature `ID`, meetings a document id, news a WP post id) on first sight and never changes it. A renamed project keeps its URL and just changes its title, so no redirect is needed. The registry is committed; only the daily workflow writes it (`contents: write`). Never mint a slug from anything that can reorder — a positional counter would move indexed URLs whenever the feed shuffles. Colliding names get a stable discriminator (suburb, then Council's id): Council genuinely publishes several distinct works under one name — seven "Redbank Plains Road- Road Resurfacing" jobs across three suburbs, which used to overwrite each other into a single page.
- `site/404.html` catches what the registry can't (retired projects, typos, rotted links) and puts the search widget in front of them. GitHub Pages serves it automatically on a custom domain. It's deliberately kept out of the sitemap. Note `_redirects` is a Cloudflare/Netlify feature and does nothing on GitHub Pages.
